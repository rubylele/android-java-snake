...
public class Snake {
    ...
    private ArrayList<PartofSnake> arrPartSnake = new ArrayList<>();
    private int length;
    ...
    public Snake(Bitmap bm, int x, int y, int length) {
        ...
        setMove_right(true);
        arrPartSnake.add(new PartofSnake(bm_head_right, x, y));
        for (int i = 1; i < length-1; i++){
            this.arrPartSnake.add(new PartofSnake(bm_body_horizontal, this.arrPartSnake.get(i-1).getX()-GameView.sizeElementMap, y));
        }
        arrPartSnake.add(new PartofSnake(bm_tail_right, arrPartSnake.get(length-2).getX()-GameView.sizeElementMap, arrPartSnake.get(length-2).getY()));
    }
    ...
    public ArrayList<PartofSnake> getArrPartSnake() {
        return arrPartSnake;
    }

    public void setArrPartSnake(ArrayList<PartofSnake> arrPartSnake) {
    ...
    public void addPart() {
        PartofSnake p = this.arrPartSnake.get(length-1);
        this.length += 1;
        if(p.getBm()==bm_tail_right){
            this.arrPartSnake.add(new PartofSnake(bm_tail_right, p.getX()-GameView.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_left){
            this.arrPartSnake.add(new PartofSnake(bm_tail_left, p.getX()+GameView.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_up){
            this.arrPartSnake.add(new PartofSnake(bm_tail_up, p.getX(), p.getY()+GameView.sizeElementMap));
